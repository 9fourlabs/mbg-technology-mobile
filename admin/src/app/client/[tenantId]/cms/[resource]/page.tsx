...
interface Props {
  params: Promise<{ tenantId: string; resource: string }>;
}

/**
 * List view for a CMS resource (e.g., /client/mbg/cms/events).
 * The Refine app handles data fetching client-side; this server component
 * just does the auth check + tenant lookup.
 */
export default async function ResourceListPage({ params }: Props) {
  const { tenantId, resource: resourceName } = await params;
